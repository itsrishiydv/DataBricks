...
if spark._jsparkSession.catalog().tableExists("f1_processed.circuits"):
    old_circuit_df = spark.table("f1_processed.circuits")
    final_circuit_df = final_circuit_df.join(old_circuit_df,final_circuit_df.circuit_id == old_circuit_df.circuitId, 'anti')
    display(final_circuit_df)
...
final_circuit_df.write.mode("append").format("parquet").saveAsTable("f1_processed.circuits")
...
race_final_df.write.format("parquet").mode("overwrite").saveAsTable("f1_processed.races")
